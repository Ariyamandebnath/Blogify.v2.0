...
async function readPublicKey(): Promise<string>{
    return promisify(readFile)(
        path.join( __dirname, '../../../keys/public_key.pem'), 'utf8');
...
async function readPrivateKey(): Promise<string>{
    return promisify(readFile)(
        path.join(__dirname, '../../../keys/private_key.pem'), 'utf8');
...
//This function validates and returns data if token is valid in all respect
...
//returns the decoded payload if the signature is valid even if the signature is expired

async function decode(): Promise<JwtPayload>{
    const cert = await readPublicKey();
